...
import { defaultEntitySearchSettings, searchEngineKey, type EntitySearchEngineId, type EntitySearchSettings } from './entitySearchSettings'
...
type SearchPage = { title?: string; langlinks?: Array<{ title?: string; '*'?: string }> }
type SearchResponse = { query?: { pages?: Record<string, SearchPage> } }
type TitleResponse = { query?: { pages?: Record<string, { title?: string }> } }
type WikidataResponse = { search?: Array<{ label?: string; match?: { text?: string }; display?: { label?: { value?: string } }; concepturi?: string }> }
type WikiSearchResponse = { query?: { search?: Array<{ title?: string; snippet?: string }> } }
...
export class WikimediaEntityLookup implements EntityLookupProvider {
  ...
  async lookup(source: string, context: EntityLookupContext = {}): Promise<EntityLookupResult> {
    ...
    url.search = new URLSearchParams({ action: 'query', format: 'json', origin: '*', generator: 'search', gsrsearch: source, gsrnamespace: '0', gsrlimit: '5', prop: 'langlinks', lllang: 'zh', lllimit: 'max' }).toString()
    const response = await this.request(url, { headers: { 'Api-User-Agent': 'NSTrans/0.1.0 (game translation terminology lookup)' } })
    ...
    return { source, target: chineseTitle, status: 'learned', sourceUrl: `https://ja.wikipedia.org/wiki/${encodeURIComponent(exact.title!.replace(/ /gu, '_'))}` }
  }

  private async toSimplifiedChineseTitle(title: string) {
    const url = new URL('https://zh.wikipedia.org/w/api.php')
    url.search = new URLSearchParams({ action: 'query', format: 'json', origin: '*', titles: title, redirects: '1', converttitles: '1', variant: 'zh-cn' }).toString()
    const response = await this.request(url, { headers: { 'Api-User-Agent': 'NSTrans/0.1.0 (game translation terminology lookup)' } })
  ...
  private async lookupWikidata(source: string, wikipediaCandidate?: SearchPage): Promise<EntityLookupResult> {
    ...
    url.search = new URLSearchParams({ action: 'wbsearchentities', format: 'json', origin: '*', search: source, language: 'ja', uselang: 'zh-cn', type: 'item', limit: '5' }).toString()
    const response = await this.request(url, { headers: { 'Api-User-Agent': 'NSTrans/0.1.0 (game translation terminology lookup)' } })
    ...
    if (exact && target && normalizeMemoryText(target) !== normalizeMemoryText(source)) return { source, target, status: 'learned', sourceUrl: exact.concepturi?.replace(/^http:/u, 'https:') }
  ...
  private async contextualSearch(source: string, gameNames: readonly string[], fallback: EntityLookupResult): Promise<EntityLookupResult> {
    ...
    const searches = await Promise.all(['ja', 'zh'].map(async (language, index) => {
      ...
      try {
        const response = await this.request(url, { headers: { 'Api-User-Agent': 'NSTrans/0.1.0 (game translation terminology research)' } })
      ...
      } catch { return { query, evidence: [], sourceUrls: [] } }
    }))
    ...
    return { ...fallback, source, research, sourceUrl: research.sourceUrls[0] ?? fallback.sourceUrl }
  }
}
...
export class HttpEntityWebSearchTransport implements EntityWebSearchTransport {
  ...
  private readonly request: typeof fetch
  constructor(endpoint = '/api/entity-search', request: typeof fetch = fetch) { this.endpoint = endpoint; this.request = request.bind(globalThis) }
  async search(engine: Exclude<EntitySearchEngineId, 'wiki'>, query: string, apiKey: string) {
    if (isTauri()) return await invoke<EntitySearchHit[]>('entity_web_search', { engine, query, apiKey })
    const response = await this.request(this.endpoint, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ engine, query, apiKey }) })
    const body = await response.json() as { hits?: EntitySearchHit[]; error?: string }
...
export class ConfigurableEntityLookup implements EntityLookupProvider {
  ...
  constructor(wiki: EntityLookupProvider = new WikimediaEntityLookup(), web: EntityWebSearchTransport = new HttpEntityWebSearchTransport()) { this.wiki = wiki; this.web = web }
  ...
  private async lookupWeb(engine: Exclude<EntitySearchEngineId, 'wiki'>, source: string, gameNames: readonly string[], apiKey: string): Promise<EntityLookupResult> {
    ...
    return {
      ...
      research: { term: source, query, evidence: hits.map(({ title, snippet }) => `${title}: ${snippet}`.trim()).slice(0, 6), sourceUrls: hits.map(({ url }) => url).slice(0, 6) },
    }
  }
}

function searchEngineLabel(engine: EntitySearchEngineId) {
  return ({ wiki: 'Wikipedia / Wikidata', brave: 'Brave Search', qianfan: '百度千帆' } as const)[engine]
}
...
const stripMarkup = (text: string) => text.replace(/<[^>]*>/gu, '').replace(/&quot;/gu, '"').replace(/&#0?39;/gu, "'").replace(/&amp;/gu, '&').replace(/\s+/gu, ' ').trim()
...
export class EntityLearningQueue {
  ...
  constructor(memory: TranslationMemory, provider: EntityLookupProvider) { this.memory = memory; this.provider = provider }

  schedule(sources: readonly string[], gameId: GameId) {
    void this.resolve(sources.flatMap(extractKatakanaCandidates), gameId, [], 0)
  }

  async resolve(candidates: readonly (string | EntityLookupCandidate)[], gameId: GameId, gameNames: readonly string[] = [], timeoutMs = 3500, searchSettings: EntitySearchSettings = defaultEntitySearchSettings) {

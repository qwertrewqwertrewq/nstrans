...
function App() {
  ...
  return <main className={`app-shell ${mobileClient ? 'android-client' : ''} ${clientPlatform === 'ios' ? 'ios-client' : ''} ${expandedPreview ? 'preview-expanded' : ''} ${fullscreenPreview ? 'preview-fullscreen' : ''}`}>
    <header className="topbar"><div className="brand"><span className="brand-mark"><img src="/logo.png" alt="NSTrans" className="brand-logo" /></span><span>NSTrans</span><small>LIVE OCR</small></div><div className="top-status"><span className={`status-dot ${running ? 'live' : ''}`} />{running ? '实时处理中' : '待机'}<span className="divider" />日本語 → 简体中文</div><button className="icon-button" title="设置"><Settings2 size={18} /></button></header>

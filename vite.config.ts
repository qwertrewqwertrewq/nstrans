...
function getMeikiCommand(): { binary: string; args: string[] } {
  if (process.platform === 'win32') {
    if (existsSync('native/runtime/windows/nstrans-meiki-ocr.exe')) {
      return {
        binary: resolve('native/runtime/windows/nstrans-meiki-ocr.exe'),
        args: [],
      }
    }
    if (existsSync('.build/windows/venv/Scripts/python.exe')) {
      return {
        binary: resolve('.build/windows/venv/Scripts/python.exe'),
        args: ['native/ocr/meiki_worker.py'],
      }
    }
  }
...
function runMeiki(image: Uint8Array): Promise<string | null> {
  if (!meikiWorker) {
    ...
    meikiWorker.on('error', () => {
      meikiWorker = null
      while (meikiPending.length) meikiPending.shift()?.(null)
    })
    meikiWorker.on('close', () => {
      meikiWorker = null
      while (meikiPending.length) meikiPending.shift()?.(null)
    })
  }
  return new Promise((resolve) => {
    ...
    meikiWorker!.stdin.write(`${JSON.stringify({ image: Buffer.from(image).toString('base64'), det_threshold: 0.45, rec_threshold: 0.15 })}\n`)
  })
}
...
async function ollamaAvailable() {
  try {
    ...
    return response.ok && Boolean(data.models?.some(({ name }) => name.startsWith('translategemma:4b')))
  } catch {
    return false
  }
}
...
async function ollamaServerAvailable() {
  try {
    return (await fetch('http://127.0.0.1:11434/api/tags')).ok
  } catch {
    return false
  }
}
...
async function ensureOllama() {
  ...
  ollamaStartup = new Promise<boolean>((resolve, reject) => {
    ...
    const check = async () => {
      ...
      if (++attempts >= 40) {
        reject(new Error('Ollama 启动超时；请打开 Ollama 应用后重试'))
        return
      }
      setTimeout(() => {
        void check()
...
async function runTranslateGemma(body: string) {
  ...
  const payload = JSON.parse(body) as {
    texts?: string[]
    history?: Array<{ sources: string[]; translations: string[] }>
  ...
  for (const source of texts) {
    ...
    context.push(`${source} → ${translation}`)
  }
...
async function runEntitySearch(body: string) {
  ...
  if (payload.engine === 'brave') {
    ...
    const searchResponse = await fetch('https://qianfan.baidubce.com/v2/ai_search/web_search', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        messages: [{ role: 'user', content: [...query].slice(0, 60).join('') }],
  ...
  console.info('[entity-search-proxy]', {
...
async function runQwenFlash(body: string) {
  const payload = JSON.parse(body) as {
    model?: string
    apiKey?: string
    prompt?: string
  ...
  if (!payload.model || !/^[\w./:-]{1,120}$/u.test(payload.model)) throw new Error('模型 ID 格式无效')
  if (!payload.apiKey?.trim() || !payload.prompt?.trim()) throw new Error('Qwen API Key 或提示词为空')
  const content = payload.imageDataUrl ? [{ image: payload.imageDataUrl }, { text: payload.prompt }] : [{ text: payload.prompt }]
  const enableSearch = Boolean(payload.enableSearch)
  if (payload.searchOnly && payload.imageDataUrl) throw new Error('仅搜索模型不能接收截图')
  const endpoint = payload.endpoint?.startsWith('https://') ? payload.endpoint : payload.searchOnly ? 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions' : 'https://dashscope.aliyuncs.com/api/v1/services/aigc/multimodal-generation/generation'
  const requestBody = payload.searchOnly
    ? {
        model: payload.model,
    ...
    : {
        ...
        parameters: {
          temperature: 0.1,
          enable_search: enableSearch,
          incremental_output: enableSearch,
          response_format: { type: 'json_object' },
        },
      }
  const response = await fetch(endpoint, {
    ...
    headers: {
      ...
      'content-type': 'application/json',
      ...(!payload.searchOnly && enableSearch ? { 'X-DashScope-SSE': 'enable' } : {}),
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(error.message || `Qwen Flash 请求失败 (${response.status})`)
  }
  const extract = (value: unknown) =>
    (
      ...
      .map(({ text }) => text ?? '')
      .join('')
  const output = payload.searchOnly
    ? ((
        JSON.parse(raw) as {
    ...
    : enableSearch
      ? raw
          ...
          .map((line) => {
            try {
              return extract(JSON.parse(line.slice(5)))
            } catch {
              return ''
            }
          })
  ...
  if (!output.trim()) throw new Error('Qwen Flash 没有返回结果')
...
function runNative(binary: string, args: string[], input: string | Uint8Array = ''): Promise<{ output: string; status: number }> {
  return new Promise((resolve) => {
    ...
    child.on('error', (error) =>
      resolve({
        output: JSON.stringify({ available: false, error: error.message }),
        status: 1,
      }),
    )
...
function macTranslationPlugin(): Plugin {
  return {
    name: 'nstrans-macos-translation',
    configureServer(server) {
      server.middlewares.use('/api/macos-translation/status', async (_request, response) => {
        const result =
          process.platform === 'darwin'
            ? await runNativeTranslation(['--status'])
            : {
                output: JSON.stringify({
                  available: false,
                  error: '仅 macOS 支持系统翻译。',
                }),
                status: 1,
              }
        response.statusCode = result.status === 0 ? 200 : 503
      ...
      server.middlewares.use('/api/macos-ocr', (request, response, next) => {
        if (request.method !== 'POST' || process.platform !== 'darwin') {
          next()
          return
        }
        ...
        request.on('end', async () => {
          ...
          response.statusCode = result.status === 0 ? 200 : 500
          response.setHeader('Content-Type', 'application/json; charset=utf-8')
          response.end(result.output)
        })
      })
      server.middlewares.use('/api/meiki-ocr', (request, response, next) => {
        ...
        request.on('end', async () => {
          ...
          response.statusCode = output ? 200 : 503
          response.setHeader('Content-Type', 'application/json; charset=utf-8')
          response.end(output ?? JSON.stringify({ regions: [], error: 'meikiocr 服务不可用' }))
        })
      })
      server.middlewares.use('/api/translategemma/status', async (_request, response) => {
        try {
          ...
          response.end(
            JSON.stringify({
              available: false,
              error: error instanceof Error ? error.message : 'Ollama 启动失败',
            }),
          )
        }
      })
      server.middlewares.use('/api/entity-search', (request, response, next) => {
        ...
        void readBody(request)
          ...
          .catch((error: unknown) => {
            console.warn('[entity-search-proxy]', {
              status: 'error',
      ...
      server.middlewares.use('/api/qwen-flash', (request, response, next) => {
        ...
        void readBody(request)
          ...
          .catch((error: unknown) => {
            ...
            response.end(
              JSON.stringify({
                error: error instanceof Error ? error.message : 'Qwen Flash 服务不可用',
              }),
            )
          })
      })
      server.middlewares.use('/api/translategemma', (request, response, next) => {
        ...
        void readBody(request)
          ...
          .catch((error: unknown) => {
            ...
            response.end(
              JSON.stringify({
                error: error instanceof Error ? error.message : 'TranslateGemma 服务不可用',
              }),
            )
          })
      })

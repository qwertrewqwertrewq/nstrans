...
- 视频画面、截图和完整 OCR 识别文本只在客户端即时处理，不上传至 NSTrans 社区服务器，服务器也不保存用户截取画面中的对白、剧情文本或画面内容。用户主动开启“远程视觉识别”时，只有搜索链全部为空的当前文字局部截图、该区域 OCR 文本和游戏名称会直接发送给用户配置的远程多模态模型；该开关默认关闭。
...
- 核心翻译模型可独立选择本机 TranslateGemma 4B 或远程 LLM。远程核心模型可从所有在线模型配置中选择；多模态模型和仅搜索能力模型都可用于纯文本核心翻译，但仅多模态模型能接收 OCR 原文本及局部截图，离线模型配置暂不参与路由
